...
const SeasonStat = ({ label, value }) => (
  <motion.div
    ...
    className="flex flex-col gap-1 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur"
  >
    <p className="text-[10px] uppercase tracking-[0.32em] text-blue-100/70">{label}</p>
...
export const DriverStats = ({ seasonStats, careerStats }) => {
  const seasonData = [
  ...
  return (
    <section className="mt-12 grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3 text-white">
          <div>
            ...
            <h3 className="text-2xl font-semibold">2026 Season</h3>
          </div>
        </div>
      ...
      <motion.div
        ...
        viewport={{ once: true, amount: 0.3 }}
        className="relative flex flex-col gap-3 overflow-hidden rounded-3xl border border-white/10 bg-[linear-gradient(145deg,rgba(15,21,36,0.9),rgba(16,27,52,0.9))] p-6 backdrop-blur"
      >
        ...
        <div className="flex items-center justify-between text-white">
          <div>
            ...
            <h3 className="text-2xl font-semibold">Alexander Albon</h3>
          </div>
        </div>
